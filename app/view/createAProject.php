...
?>
<h1><?= $title ?></h1>
<p>Vous devez être membre d'un groupe pour créer un projet.</p>
<?php if (empty($groups)) { //if no groups found, the user can't create a project
    flshmsg(CREATEAPROJECT_NO_GROUPS_VIEW_FAIL);
    $disabledIfNoGroups = " disabled ";
} ?>
<form action="?action=createAProject" method="POST" class="pt-1">
    <div class="divContext row pt-2 box-verticalaligncenter">
        <div class="col-lg-3 col-md-12 ">
            ...
            <input type="text" class="form-control smalltextinput textFieldToCheck" name="name" id="txtName"
                   maxlength="70" required value="<?= $data['name'] ?>" <?= $disabledIfNoGroups ?>>
        </div>

        <div class="col-lg-3 col-md-10">
            <h4>Réalisé par</h4>
            <div class="d-inline-flex box-verticalaligncenter">
                <select name="manager_id" id="manager_id" class="form-control" required <?= $disabledIfNoGroups ?>>
    ...
    <div class="divDescription row pt-4">
        ...
        <div class="col-12"><textarea class="form-control fullwidth textFieldToCheck" name="description"
                                      ...
                                      required <?= $disabledIfNoGroups ?>><?= $data['description'] ?></textarea>
        </div>
    ...
    <div class="divContext row pt-2">
        ...
        <div class="col-12"><textarea class="form-control fullwidth textFieldToCheck" name="goal" id="txtGoal" rows="2"
                                      maxlength="500"
                                      minlength="3" <?= $disabledIfNoGroups ?>><?= $data['goal'] ?></textarea></div>
    </div>
    ...
    <div class="divContext row pt-2">
        ...
        <div class="ml-4">
            ...
            <input type="date" id="start" name="start"
                   ...
                   required <?= $disabledIfNoGroups ?>>
        </div>
        <div class="ml-4">
            <label for="end">Fin (facultatif):</label>
            <input class="marginauto" type="date" id="end" min="" name="end"
                   value="<?= ((($data['end']) != '') ? date("Y-m-d", strtotime($data['end'])) : "") ?>" <?= $disabledIfNoGroups ?>>
        </div>
    ...
    <div class="divImportance row">
        <div class="col-lg-3 col-md">
            ...
            <input class="form-control inputtypenumber" type="number" min="1" max="5" name="importance"
                   id="selImportance" value="<?= ((isset($data['importance'])) ? $data['importance'] : 1) ?>"
                   required <?= $disabledIfNoGroups ?>>
        </div>
        <div class="col-lg-3 col-md">
            <h4>Urgence</h4>
            <input class="form-control inputtypenumber" type="number" min="1" max="5" name="urgency" id="selUrgency"
                   value="<?= ((isset($data['urgency'])) ? $data['urgency'] : 1) ?>"
                   required <?= $disabledIfNoGroups ?>></div>
    </div>
        ...
        <div class="mr-3">
            ...
            <div class="d-inline-flex box-verticalaligncenter">
                <input class="form-control fullwidth smalltextinput" type="password" name="password"
                       placeholder="Mot de passe" required <?= $disabledIfNoGroups ?>>
    ...
    <div class="divBtnCreate pt-4">
        <input type="submit" class="btn btn-primary" value="Créer le projet" <?= $disabledIfNoGroups ?>>
    </div>
</form>
<br><br>
<?php
